...
Эти преобразования позволили немного уменьшить объем памяти, используемый DataFrame `df_article`, с **8.9 MB** до оптимизации до **8.5 MB** после оптимизации. 
...
2. **Преобразование столбцов `club_member_status` и `fashion_news_frequency`  из `object` в категориальный тип данных (`category`):**
...
3. **Преобразование столбца `age` из  `float64` в `uint8`:**
...
Эти преобразования позволили значительно уменьшить объем памяти, используемый DataFrame `df_customers`, с **62.8 МB** до оптимизации до **26.2 MB** после оптимизации.
...
2. **Преобразование столбца `article_id` в `int32`:**
...
3. **Преобразование столбца `price` в `float32`:**
...
4. **Преобразование столбца `sales_channel_id` в `int8`:**
...
Эти преобразования позволили значительно уменьшить объем памяти, используемый DataFrame `df_transactions`, с **1.2 GB** до оптимизации до **757.9 MB** после оптимизации, что улучшает общую производительность системы при работе с большими объемами данных.
...
![ПродажиПоМесяцамЖенскаяОдежда](https://github.com/OlhaAD/EDA_IDA_H_and_M_Python/blob/main/visualizations/VerkaufssummeProMonatIndex%26Ladieswear.png)
...
- Максимальные значения и для суммы покупок, и для количества покупок увеличились. Это говорит о том, что небольшая группа клиентов совершила значительно больше покупок и потратила больше денег во втором периоде по сравнению с первым. Необходимо изучить, что побудило небольшую группу клиентов к экстремально высокой покупательской активности.
